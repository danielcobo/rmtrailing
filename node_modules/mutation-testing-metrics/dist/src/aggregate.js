...
function aggregateResultsByModule(resultsByModule) {
    ...
    const aggregatedResult = {
        ...
        projectRoot: projectRoots.length ? helpers_1.determineCommonBasePath(projectRoots) : undefined,
        config: {},
    };
    return Object.entries(resultsByModule).reduce((acc, [moduleName, report]) => {
        var _a;
        Object.entries(helpers_1.normalizeFileNames(report.files)).forEach(([fileName, fileResult]) => {
        ...
        if (report.testFiles) {
            ...
            Object.entries(helpers_1.normalizeFileNames(report.testFiles)).forEach(([fileName, testFileResult]) => {
