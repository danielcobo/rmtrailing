...
class MutationTestExecutor {
    ...
    async execute() {
        ...
        const results = await rxjs_1.lastValueFrom(rxjs_1.merge(testRunnerResult$, checkResult$, noCoverageResult$, ignoredResult$).pipe(operators_1.toArray()));
    ...
    executeIgnore(input$) {
        const [ignoredMutant$, notIgnoredMutant$] = rxjs_1.partition(input$.pipe(operators_1.shareReplay()), (mutant) => mutant.status === core_1.MutantStatus.Ignored);
        const ignoredResult$ = ignoredMutant$.pipe(operators_1.map((mutant) => this.mutationTestReportHelper.reportMutantStatus(mutant, core_1.MutantStatus.Ignored)));
        return { ignoredResult$, notIgnoredMutant$ };
    }
    executeNoCoverage(input$) {
        const [noCoverageMatchedMutant$, coveredMutant$] = rxjs_1.partition(input$.pipe(operators_1.shareReplay()), (mutant) => { var _a, _b; return !mutant.static && ((_b = (_a = mutant.coveredBy) === null || _a === void 0 ? void 0 : _a.length) !== null && _b !== void 0 ? _b : 0) === 0; });
        const noCoverageResult$ = noCoverageMatchedMutant$.pipe(operators_1.map((mutant) => this.mutationTestReportHelper.reportMutantStatus(mutant, core_1.MutantStatus.NoCoverage)));
    ...
    executeCheck(input$) {
        ...
        const [passedCheckResult$, failedCheckResult$] = rxjs_1.partition(checkTask$, ({ checkResult }) => checkResult.status === check_1.CheckStatus.Passed);
        const checkResult$ = failedCheckResult$.pipe(operators_1.map((failedMutant) => this.mutationTestReportHelper.reportCheckFailed(failedMutant.mutant, failedMutant.checkResult)));
        const passedMutant$ = passedCheckResult$.pipe(operators_1.map(({ mutant }) => mutant));
