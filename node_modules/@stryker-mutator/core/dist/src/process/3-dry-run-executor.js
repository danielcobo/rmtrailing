...
class DryRunExecutor {
    constructor(injector, log, options, timer, concurrencyTokenProvider, sandbox) {
    ...
    async execute() {
        const testRunnerInjector = this.injector
            ...
            .provideFactory(di_1.coreTokens.testRunnerPool, concurrent_1.createTestRunnerPool);
        const testRunnerPool = testRunnerInjector.resolve(di_1.coreTokens.testRunnerPool);
        const { dryRunResult, timing } = await rxjs_1.lastValueFrom(testRunnerPool.schedule(rxjs_1.of(0), (testRunner) => this.timeDryRun(testRunner)));
    ...
    remapSandboxFilesToOriginalFiles(dryRunResult) {
        const disableTypeCheckingFileMatcher = new config_1.FileMatcher(this.options.disableTypeChecks);
    ...
    logErrorsInInitialRun(runResult) {
        const message = `One or more tests resulted in an error:${os_1.EOL}\t${runResult.errorMessage}`;
        this.log.error(message);
    }
...
DryRunExecutor.inject = plugin_1.tokens(plugin_1.commonTokens.injector, plugin_1.commonTokens.logger, plugin_1.commonTokens.options, di_1.coreTokens.timer, di_1.coreTokens.concurrencyTokenProvider, di_1.coreTokens.sandbox);
