...
class Pool {
    constructor(factory, concurrencyToken$) {
        ...
        this.resource$ = concurrencyToken$.pipe(operators_1.mergeMap(async () => {
            var _a;
            if (this.isDisposed) {
                return null;
            }
     ...
     */
    async init() {
        await rxjs_1.lastValueFrom(this.resource$);
    }
    /**
    ...
    schedule(input$, task) {
        ...
        const resource$ = rxjs_1.merge(recycleBin, this.resource$);
        return rxjs_1.zip(resource$, input$).pipe(operators_1.mergeMap(async ([resource, input]) => {
