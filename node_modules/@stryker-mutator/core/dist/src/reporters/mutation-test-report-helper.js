...
class MutationTestReportHelper {
    ...
    reportMutantRunResult(mutant, result) {
        switch (result.status) {
            ...
            case test_runner_1.MutantRunStatus.Timeout:
                return this.reportOne({
                    ...mutant,
                    status: core_1.MutantStatus.Timeout,
                    statusReason: result.reason,
                });
            case test_runner_1.MutantRunStatus.Survived:
                return this.reportOne({
                    ...mutant,
                    status: core_1.MutantStatus.Survived,
                    testsCompleted: result.nrOfTests,
                });
        }
    }
    ...
    checkStatusToResultStatus(status) {
        switch (status) {
            case check_1.CheckStatus.CompileError:
                return core_1.MutantStatus.CompileError;
        }
    }
    reportAll(results) {
        const report = this.mutationTestReport(results);
        const metrics = mutation_testing_metrics_1.calculateMutationTestMetrics(report);
    ...
    toFileResults(results, remapTestIds) {
        return results.reduce((acc, mutantResult) => {
            var _a;
            const fileResult = (_a = acc[mutantResult.fileName]) !== null && _a !== void 0 ? _a : (acc[mutantResult.fileName] = this.toFileResult(mutantResult.fileName));
            fileResult.mutants.push(this.toMutantResult(mutantResult, remapTestIds));
            return acc;
        }, {});
    }
    toTestFiles(remapTestId) {
        return this.dryRunResult.tests.reduce((acc, testResult) => {
            ...
            const testFile = (_b = acc[fileName]) !== null && _b !== void 0 ? _b : (acc[fileName] = this.toTestFile(fileName));
            testFile.tests.push(test);
            return acc;
        }, {});
    }
    toFileResult(fileName) {
        const fileResult = {
        ...
        if (sourceFile) {
            ...
            this.log.warn(util_1.normalizeWhitespaces(`File "${fileName}" not found
    ...
    toTestFile(fileName) {
        ...
        if (fileName) {
            ...
            if (sourceFile) {
                ...
                this.log.warn(util_1.normalizeWhitespaces(`Test file "${fileName}" not found
    ...
    discoverDependencies() {
        const discover = (specifier) => {
            try {
                return [specifier, util_1.requireResolve(`${specifier}/package.json`).version];
            }
            catch {
