...
class InputFileResolver {
    ...
    resolveMutateFiles(inputFileNames) {
        return this.filterPatterns(inputFileNames, this.mutatePatterns, !util_1.isDeepStrictEqual(this.mutatePatterns, options_validator_1.defaultOptions().mutate));
    }
    ...
    async readFiles(fileNames) {
        const files$ = rxjs_1.from(fileNames).pipe(operators_1.mergeMap((fileName) => {
            return this.readFile(fileName);
        }, file_utils_1.MAX_CONCURRENT_FILE_IO), operators_1.filter(util_2.notEmpty), operators_1.toArray(), 
    ...
    async readFile(fileName) {
        try {
            ...
            if ((util_2.isErrnoException(error) && error.code === 'ENOENT') || error.code === 'EISDIR') {
