...
class ChildProcessProxy {
    constructor(requirePath, requireName, loggingContext, options, additionalInjectableValues, workingDirectory, execArgv) {
        ...
        this.handleUnexpectedExit = (code, signal) => {
            ...
            function stdoutAndStderr() {
                if (output.length) {
                    return `Last part of stdout and stderr was:${os_1.default.EOL}${string_utils_1.padLeft(output)}`;
                }
                else {
        ...
        this.worker = child_process_1.default.fork(require.resolve('./child-process-proxy-worker'), [message_protocol_1.autoStart], { silent: true, execArgv });
    ...
    send(message) {
        this.worker.send(string_utils_1.serialize(message));
    }
    initProxy() {
    ...
    listenForMessages() {
        this.worker.on('message', (serializedMessage) => {
            const message = string_utils_1.deserialize(serializedMessage);
            switch (message.kind) {
    ...
    async dispose() {
        if (!this.isDisposed) {
            ...
            try {
                ...
                await object_utils_1.kill(this.worker.pid);
            }
        }
    }
