...
const transformBabel = ({ root, originFileName, rawContent, offset }, mutantCollector, { options }, mutators = mutators_1.allMutators, mutantPlacers = mutant_placers_1.allMutantPlacers) => {
    ...
    core_1.traverse(file.ast, {
        enter(path) {
            if (shouldSkip(path)) {
                ...
                if (!mutantRangesForCurrentFile.length || mutantRangesForCurrentFile.some((range) => syntax_helpers_1.locationIncluded(range, path.node.loc))) {
    ...
    function shouldSkip(path) {
        return (syntax_helpers_1.isTypeNode(path) ||
    ...
    function placeMutantsIfNeeded(path) {
        ...
        if (mutantsPlacement === null || mutantsPlacement === void 0 ? void 0 : mutantsPlacement.appliedMutants.size) {
            try {
                ...
                mutant_placers_1.throwPlacementError(error, path, mutantsPlacement.placer, [...mutantsPlacement.appliedMutants.keys()], originFileName);
            }
        }
    }
    /**
