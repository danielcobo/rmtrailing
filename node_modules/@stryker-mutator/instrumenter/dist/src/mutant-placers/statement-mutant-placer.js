...
exports.statementMutantPlacer = {
    ...
    place(path, appliedMutants) {
        let statement = core_1.types.blockStatement([
            core_1.types.expressionStatement(syntax_helpers_1.mutationCoverageSequenceExpression(appliedMutants.keys())),
            ...(path.isBlockStatement() ? path.node.body : [path.node]),
        ]);
        for (const [mutant, appliedMutant] of appliedMutants) {
            statement = core_1.types.ifStatement(syntax_helpers_1.mutantTestExpression(mutant.id), core_1.types.blockStatement([appliedMutant]), statement);
        }
