...
exports.switchCaseMutantPlacer = {
    ...
    place(path, appliedMutants) {
        let consequence = core_1.types.blockStatement([
            core_1.types.expressionStatement(util_1.mutationCoverageSequenceExpression(appliedMutants.keys())),
            ...path.node.consequent,
        ]);
        for (const [mutant, appliedMutant] of appliedMutants) {
            consequence = core_1.types.ifStatement(util_1.mutantTestExpression(mutant.id), core_1.types.blockStatement(appliedMutant.consequent), consequence);
        }
